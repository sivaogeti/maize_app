...
void _nav(BuildContext context, String route) {
  try {
    if (route.startsWith('/')) {
      // It's a GoRouter path like '/farmers/network'
      context.push(route);
    } else {
      // It's a GoRouter named route like 'ci.field_incharges.inputs-details.detail'
      context.pushNamed(route);
    }
  } catch (e) {
    debugPrint('[Nav] failed for $route: $e');
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('Navigation failed: $route')),
    );
  }
}
...
// =====================
      ...
      if (isCIC) ...[
        ...
        _DashTile(
          ...
          route: '/cic/field-incharges/farmer-networks',
        ),
        _DashTile(
          label: 'Farmers Registration Details',
          color: const Color(0xFFE0F2F1),
          icon: Icons.app_registration_outlined,
          route: '/cic/field-incharges/farmer-registrations',
        ),

        _DashTile(
          label: 'Field Incharge Inputs Details',
          color: const Color(0xFFF7E1CC),
          icon: Icons.science_outlined,
          onTap: () {
            // Navigate to FI selection screen, do NOT pass a UID yet
            context.push('/cic/field-incharges/inputs-details');
          },
        ),
      ...
      child: InkWell(
        ...
        onTap: () {
          if (onTap != null) {
            ...
            _nav(context, route!); // ✅ unified navigation
          }
        },
        child: Container(
