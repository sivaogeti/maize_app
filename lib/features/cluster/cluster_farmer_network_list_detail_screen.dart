...
class ClusterFarmerNetworkDetailsScreen extends StatefulWidget {
  final String uid;

  const ClusterFarmerNetworkDetailsScreen({
    Key? key,
    required this.uid,
  }) : super(key: key);

  @override
  State<ClusterFarmerNetworkDetailsScreen> createState() =>
      _ClusterFarmerRegistrationsDetailsScreenState();
}

class _ClusterFarmerRegistrationsDetailsScreenState extends State<ClusterFarmerNetworkDetailsScreen> {

  late final docRef = FirebaseFirestore.instance
      .collection('farmer_networks') // 👈 change this to your actual Firestore collection
      .doc(widget.uid);

  late final docId = widget.uid;
